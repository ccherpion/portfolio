...
function render() {
    ...
    if (d.stack) {
        document.getElementById('stack-grid').innerHTML = d.stack.map(s => `<div class="bento-card bg-white dark:bg-[#1f1f22] p-6 rounded-2xl border border-gray-100"><div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100 dark:border-zinc-800"><div class="badge-blue"><i class="${s.icon}"></i></div><h4 class="text-sm font-bold uppercase font-tech">${s.category}</h4></div><div class="grid grid-cols-2 gap-4">${s.items.map(item => `<div class="flex items-center gap-2"><img src="${item.logo}" class="w-5 h-5 dark:invert grayscale opacity-50"><span class="text-[11px] font-bold uppercase font-tech">${item.name}</span></div>`).join('')}</div></div>`).join('');
    }
    if (d.projects) {
        document.getElementById('projects-container').innerHTML = d.projects.map(p => `<div class="bento-card group relative overflow-hidden aspect-[4/3] p-0 border border-gray-100"><img src="${p.img}" class="w-full h-full object-cover transition-all duration-700 grayscale group-hover:grayscale-0 group-hover:scale-105"><div class="absolute inset-0 bg-black/60 flex flex-col justify-end p-8 text-white"><h3 class="text-3xl font-black uppercase mb-2 tracking-tighter">${p.name}</h3><p class="text-[11px] opacity-0 group-hover:opacity-100 transition-opacity mb-4">${p.desc}</p><div class="flex justify-between items-center"><span class="badge-blue !bg-white/10 !text-white !border-transparent">${p.tag}</span><span class="text-2xl font-black font-tech">${p.val}</span></div></div></div>`).join('');
    }
    if (d.experiences) {
        document.getElementById('experience-grid').innerHTML = d.experiences.map(exp => `<div class="bento-card bg-white dark:bg-[#1f1f22] p-6 rounded-2xl border border-gray-100 flex flex-col justify-between min-h-[250px]"><div><div class="flex justify-between items-start mb-5"><div class="badge-blue"><i class="${exp.icon}"></i></div><span class="text-[10px] font-bold text-gray-400 font-tech uppercase">${exp.date}</span></div><h4 class="text-lg font-bold uppercase mb-1">${exp.role}</h4><p class="text-blue-600 text-[10px] font-black uppercase mb-4">${exp.company}</p></div><p class="text-gray-500 dark:text-gray-400 text-[13px] leading-relaxed">${exp.desc}</p></div>`).join('');
    }
}
...
function handleRouting() {
    ...
    if(target) {
        ...
        if(subMap[hash]) setTimeout(() => runScramble(subMap[hash], 5), 150);
        triggerBento();
    }
